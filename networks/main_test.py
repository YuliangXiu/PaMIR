...
def main_test_wo_gt_smpl_with_optm(test_img_dir, out_dir, pretrained_checkpoint, pretrained_gcmr_checkpoint,
    ...
    for step, batch in enumerate(tqdm(loader, desc='Testing', total=len(loader), initial=0)):
        ...
        img_fname = os.path.split(img_dir)[1]
        mesh_fname = os.path.join(out_dir, img_fname[:-4] + '.obj')
        init_smpl_fname = os.path.join(out_dir, img_fname[:-4] + '_init_smpl.obj')
        optm_smpl_fname = os.path.join(out_dir, img_fname[:-4] + '_optm_smpl.obj')
        
        # export 
        Image.fromarray((255.0*batch['img'][0]).permute(1,2,0).detach().cpu().numpy().astype(np.uint8)).save(
            os.path.join(out_dir, img_fname))
        obj_io.save_obj_data(mesh, mesh_fname)
        ...
        obj_io.save_obj_data({'v': optm_smpl.squeeze().detach().cpu().numpy(), 'f': smpl_faces},
                             optm_smpl_fname)
        smpl_param_name = os.path.join(out_dir, img_fname[:-4] + '_smplparams.pkl')
